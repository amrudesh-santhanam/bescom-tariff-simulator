...
def calculate_bills(session_id=None):
    STD_RATE_LT = 140.0
    ...
    with get_connection() as conn:
        ...
        if session_id is None:
            session_id = get_latest_session_id(conn, 'factory_load_data')

        if session_id == 0:
            raise ValueError('No factory_load_data session exists in data.db.')

        df = load_session_dataframe(conn, 'factory_load_data', session_id)
        if df.empty:
            raise ValueError(f'No data found for session_id={session_id} in data.db.')
        ...
        for case in cases:
            ...
            for sl in sanctioned_loads:
                ...
                summary_standard.append({
                    'Case': case,
                    'Sanctioned_Load_kW': sl,
                    'Max_Demand_kW': round(max_demand, 2),
                    'Base_Fixed_Charge_Rs': round(base_std_fixed, 2),
                    'Penalty_Charge_Rs': round(penalty_std, 2),
                    'Total_Fixed_Charges_Rs': round(base_std_fixed + penalty_std, 2),
                    'Energy_Charge_Rs': round(total_energy_cost, 2),
                ...
                summary_demand.append({
                    ...
                    'Sanctioned_Load_kW': sl,
                    'Max_Demand_kW': round(max_demand, 2),
                    'Base_Demand_Charge_Rs': round(base_dem_fixed, 2),
                    'Penalty_Charge_Rs': round(penalty_dem, 2),
                    'Total_Demand_Charges_Rs': round(base_dem_fixed + penalty_dem, 2),
                    'Energy_Charge_Rs': round(total_energy_cost, 2),
                    'Total_Bill_Rs': round(total_dem_bill, 2)
                })

        save_dataframe_to_table(pd.DataFrame(summary_standard), 'billing_summary_standard', conn, session_id)
        save_dataframe_to_table(pd.DataFrame(summary_demand), 'billing_summary_demand', conn, session_id)
        save_dataframe_to_table(pd.DataFrame(daily_results), 'daily_profile', conn, session_id)

        print(f'Generated billing summaries and stored them in data.db for session_id={session_id}.')
